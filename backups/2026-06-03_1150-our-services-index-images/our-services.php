...
<main class="site-main page-service">
  ...
  <?php
  $media = get_template_directory_uri() . '/assets/media/';
  $card_imgs = [$media . 'cards/business.jpg', $media . 'cards/individuals.jpg'];
  $card_links = [tnl_url('our-services/for-manager'), tnl_url('our-services/for-team-member')];
  ?>
  <!-- Hero -->
  <section class="pg-hero pg-hero--brand section">
    <div class="container">
      <h1 class="pg-hero__title"><?php echo esc_html($T['hero_tagline']); ?></h1>
      <?php if (!empty($T['hero_label'])) : ?><p class="svc-hero__sub"><?php echo esc_html($T['hero_label']); ?></p><?php endif; ?>
      <div class="pg-pills">
        <?php foreach ($T['pills'] as $p) : ?>
  ...
  <div class="ic-banner">
    <div class="container">
      <figure class="ic-banner__fig"><img src="<?php echo esc_url($media . 'growing/banner1_cbb5d66449.jpg'); ?>" alt="<?php echo esc_attr($T['hero_tagline']); ?>" loading="eager"></figure>
    </div>
  </div>
  ...
  <section class="svc-block section">
    <div class="container">
      ...
      <div class="svc-cards">
        <?php foreach ($T['business'] as $ci => $c) : ?>
          <article class="svc-card svc-card--img">
            <span class="svc-card__media"><img src="<?php echo esc_url($card_imgs[$ci] ?? $card_imgs[0]); ?>" alt="<?php echo esc_attr($c['t']); ?>" loading="lazy"></span>
            <div class="svc-card__body">
              <h3 class="svc-card__title"><?php echo esc_html($c['t']); ?></h3>
              <p class="svc-card__desc"><?php echo esc_html($c['d']); ?></p>
              <a href="<?php echo esc_url($card_links[$ci] ?? '#'); ?>" class="svc-card__more"><?php echo esc_html($T['learn_more']); ?></a>
            </div>
          </article>
        <?php endforeach; ?>
      </div>
  ...
  <section class="svc-block svc-block--alt section">
    <div class="container">
      ...
      <div class="svc-cards svc-cards--single">
        <article class="svc-card svc-card--img">
          <span class="svc-card__media"><img src="<?php echo esc_url($media . 'cards/supportive.jpg'); ?>" alt="<?php echo esc_attr($T['individual']['t']); ?>" loading="lazy"></span>
          <div class="svc-card__body">
            <h3 class="svc-card__title"><?php echo esc_html($T['individual']['t']); ?></h3>
            <p class="svc-card__desc"><?php echo esc_html($T['individual']['d']); ?></p>
            <a href="<?php echo esc_url(tnl_url('our-services/executive-coach')); ?>" class="svc-card__more"><?php echo esc_html($T['learn_more']); ?></a>
          </div>
        </article>
      </div>
      ...
      <ul class="svc-courses">
        <?php foreach ($T['courses'] as $course) : ?>
          <li class="svc-course"><a href="<?php echo esc_url(tnl_url('our-services/individual-courses')); ?>"><span><?php echo esc_html($course); ?></span><span class="svc-course__arrow" aria-hidden="true">→</span></a></li>
        <?php endforeach; ?>
      </ul>
    </div>
  </section>
  ...
  <section class="svc-tm section">
    <div class="container">
      ...
      <div class="svc-tm__grid">
        <?php foreach ($T['people'] as $pi => $p) : $av = function_exists('tnl_avatar') ? tnl_avatar($p['n']) : ''; ?>
          <figure class="svc-quote">
            ...
            <figcaption class="svc-quote__by">
              ...
              <?php else : ?><span class="pg-quote__avatar pg-quote__avatar--initials pg-quote__avatar--<?php echo $pill_mods[$pi % 4]; ?>"><?php echo esc_html(tnl_initials($p['n'])); ?></span><?php endif; ?>
              <span class="svc-quote__meta"><span class="svc-quote__name"><?php echo esc_html($p['n']); ?></span><span class="svc-quote__role"><?php echo esc_html($p['r']); ?></span></span>
            </figcaption>
          </figure>
        <?php endforeach; ?>
      </div>
