...
/**
 * Cây menu (mega) theo ngôn ngữ — khớp cấu trúc nav trang live.
 * 'slug' là đường dẫn tương đối (chưa prefix lang); có thể kèm #anchor.
 * Trang con chi tiết (phase 2) tạm trỏ về trang index cha để không 404.
 */
function tnl_nav_tree() {
    $vi = (tnl_lang() === 'vi');
    if ($vi) {
        return [
            ...
            ['label' => 'Sản phẩm & Dịch vụ', 'slug' => 'our-services', 'columns' => [
                ['heading' => ['label' => 'Chương trình đào tạo', 'slug' => 'our-services'], 'items' => [
                    ...
                    ['label' => 'Dành cho đội nhóm', 'slug' => 'our-services/for-team-member'],
                    ['label' => 'Coaching cho lãnh đạo', 'slug' => 'our-services/executive-coach'],
                    ['label' => 'Khóa cá nhân', 'slug' => 'our-services/individual-courses'],
                ]],
                ['heading' => ['label' => 'Sản phẩm', 'slug' => 'products'], 'items' => [
                    ['label' => 'Cẩm nang EQ cho quản lý, lãnh đạo', 'slug' => 'products/heart-heart-hand'],
                    ['label' => 'Bộ bài giao tiếp EQ', 'slug' => 'products#card'],
                    ['label' => 'Sách điện tử', 'slug' => 'products/the-story-of-empathy'],
                    ['label' => 'The EQ Calendar', 'slug' => 'products/the-eq-calendar'],
                ]],
            ]],
            ['label' => 'Đánh giá & Tài nguyên', 'slug' => 'resources', 'columns' => [
                ['items' => [
            ...
            ['label' => 'Sự kiện', 'slug' => 'events', 'columns' => [
                ['items' => [
                    ['label' => 'Sự kiện đang diễn ra', 'slug' => 'events#previous-events'],
                    ['label' => 'Workshop', 'slug' => 'events#previous-events'],
                    ['label' => 'Sự kiện đã diễn ra', 'slug' => 'events#previous-events'],
                ]],
            ]],
            ['label' => 'Newsletter', 'slug' => 'newsletter'],
            ['label' => 'Liên hệ', 'slug' => 'contact'],
            ['label' => 'Tuyển dụng', 'slug' => 'careers'],
        ];
    }
    return [
        ['label' => 'Home', 'slug' => ''],
        ['label' => 'Our Services & Products', 'slug' => 'our-services', 'columns' => [
            ['heading' => ['label' => 'Training programs', 'slug' => 'our-services'], 'items' => [
                ...
                ['label' => 'For Team member', 'slug' => 'our-services/for-team-member'],
                ['label' => 'Executive coach', 'slug' => 'our-services/executive-coach'],
                ['label' => 'Individual courses', 'slug' => 'our-services/individual-courses'],
            ]],
            ['heading' => ['label' => 'Our Products', 'slug' => 'products'], 'items' => [
                ['label' => 'The Emotional Intelligence (EQ) guidebook for Leaders, Managers', 'slug' => 'products/heart-heart-hand'],
                ['label' => 'The Emotional Intelligence (EQ) Decks', 'slug' => 'products#card'],
                ['label' => 'Ebook', 'slug' => 'products/the-story-of-empathy'],
                ['label' => 'The EQ Calendar', 'slug' => 'products/the-eq-calendar'],
            ]],
        ]],
        ['label' => 'Assessment & Resources', 'slug' => 'resources', 'columns' => [
            ['items' => [
        ...
        ['label' => 'Events', 'slug' => 'events', 'columns' => [
            ['items' => [
                ['label' => 'Current Event', 'slug' => 'events#previous-events'],
                ['label' => 'Workshop', 'slug' => 'events#previous-events'],
                ['label' => 'Previous Event', 'slug' => 'events#previous-events'],
            ]],
        ]],
        ['label' => 'Newsletter', 'slug' => 'newsletter'],
...
/* Build URL từ slug (có thể kèm #anchor) + prefix lang */
function tnl_nav_href($slug) {
    $anchor = '';
    if (strpos($slug, '#') !== false) {
        [$slug, $anchor] = explode('#', $slug, 2);
        $anchor = '#' . $anchor;
    }
    return tnl_url($slug) . $anchor;
}

/* Avatar testimonial theo tên (ảnh thật từ live) — '' nếu không có */
function tnl_avatar($name) {
    static $map = [
        'Nguyen Van Cong' => 'small_Cong_Nguyen_a8e583dbcb.jpg',
        'Kris van Daele'   => 'thumbnail_Kris_van_Daele_fceb0b5f8f.png',
        'Pham Thi Hoai'    => 'thumbnail_Mask_group_1_f1230bcabc.png',
        'Hoang Viet Dung'  => 'thumbnail_Hoang_Viet_Dung_e24a4efcb1.png',
        'Hoàng Việt Dũng'  => 'thumbnail_Hoang_Viet_Dung_e24a4efcb1.png',
        'Hang Nguyen'      => 'thumbnail_Hang_Nguyen_c5b954abbe.png',
        'Sarah Smith'      => 'thumbnail_sarah_0ee206e9de.png',
        'Peter Mayer'      => 'thumbnail_Peter_Mayer_a0d8b8ecd5.png',
        'Ho Minh Quang'    => 'thumbnail_Ho_Minh_Quang_64f6e23700.png',
        'Ha Thi Thuy An'   => 'thumbnail_Ha_Thi_Thuy_An_46df80b5a8.png',
        'Kathy Le'         => 'thumbnail_Kathy_Le_089e4ccb9a.png',
        'Barry Weisblatt'  => 'thumbnail_barry_2ae1594a47.png',
    ];
    return isset($map[$name]) ? get_template_directory_uri() . '/assets/media/avatars/' . $map[$name] : '';
}

/* Chữ cái đầu cho avatar fallback */
function tnl_initials($name) {
...
function tnl_strings() {
    return [
        ...
        'vi' => [
            ...
            't5_role' => 'Cựu CEO tại Tập đoàn khách sạn Lodgis, Cựu CEO Fusion Resorts & Hotels, Cựu CEO Sofitel Legend Metropole Hanoi, Cựu Phó Chủ tịch Bất động sản J.P Morgan Châu Á, MBA Harvard',
            ...
            't6_role' => "Giám đốc Nghiên cứu tại VNDIRECT Securities Corporation\nNguyên giám đốc tại Equity Markets & Securitization VinFast Global",
            't6_quote'=> 'Ngân đã thực sự giúp tôi trở thành một nhà lãnh đạo tốt hơn. Cô ấy biết lắng nghe và dựa trên kiến thức cũng như kinh nghiệm phong phú của bản thân để đưa ra những lời khuyên sâu sắc và thiết thực. Điều này giúp tôi đối mặt với các vấn đề và truyền cảm hứng cho nhóm hoạt động và phát triển một cách hiệu quả. Sau một thời gian ở vị trí dẫn đầu, chúng ta rất dễ để trở nên tự mãn trong cách làm việc. Chính vì vậy, Ngân đã giúp tôi có được góc nhìn mới và tiếp tục thăng tiến trong sự nghiệp.',
            // about
